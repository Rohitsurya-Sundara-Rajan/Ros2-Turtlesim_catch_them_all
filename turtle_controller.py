...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.alive_turtles_subscriber = self.create_subscription(TurtleAray, "alive_turtles", self.callback_alive_turtles, QoSProfile(depth=10) )
    ...
    def callback_alive_turtles(self, msg: TurtleAray):
    ...
    def control_loop(self):
        ...
        if distance > 0.5:
            ...
            #Orientation
            goal_thetha = math.atan2(dist_y, dist_x)
            diff = goal_thetha - self.pose_.theta
            
            #The following is the code for normalising the angle
            if diff > math.pi:
                diff -= 2*math.pi
            elif diff < -math.pi:
                diff += 2*math.pi
